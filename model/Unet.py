...
class Unet(tf.keras.Model):
    def __init__(self, output_classes, img_size, batch_size, input_img, input_scope):
        super().__init__()
        ...
        def pool_layer(self, prev_input, padding='SAME'):
            return tf.keras.layers.MaxPool2D(pool_size=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding=padding)(prev_input)

        def un_conv(self, prev_input, num_input_channels, conv_filter_size, num_filters, feature_map_size, train=True,
            ...
            layer = tf.keras.layers.Conv2DTranspose( num_filters, conv_filter_size, strides=2, padding=padding, output_padding=None,
            kernel_initializer=init_weights, bias_initializer=init_biases)(prev_input)
            if relu:
                layer = tf.keras.layers.ReLU(layer)
            return layer

        def conv_layer(self, prev_input, num_input_channels, conv_filter_size, num_filters, padding='SAME', relu=True):
            """
            """
            init_biases = tf.keras.initializers.Constant(value=0.05)
            init_weights = tf.keras.initializers.TruncatedNormal(mean=0.0, stddev=0.05)
            layer = tf.keras.layers.Conv2D( num_filters, conv_filter_size, strides=1, padding=padding,
        ...
        def create_unet(self):
            """
            """
            self.conv1 = self.conv_layer(self.input_net, 3, 3, 64)
            self.conv2 = self.conv_layer(self.conv1,64, 3, 64)
            self.pool2 = self.pool_layer(self.conv2)
            #
            self.conv3 = self.conv_layer(self.pool2, 64, 3, 128)
            self.conv4 = self.conv_layer(self.conv3, 128, 3, 128)
            self.pool4 = self.pool_layer(self.conv4)
            #
